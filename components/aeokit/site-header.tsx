...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-border bg-background/95 px-6 backdrop-blur supports-[backdrop-filter]:bg-background/80 lg:px-10">
      <Link href="/" className="flex items-center gap-2 text-[15px] font-semibold tracking-tight"><AEOKitLogo className="size-7 text-primary" />aeokit</Link>
      ...
      <Button className="rounded-full px-4" render={<Link href="/setup">Add to your agent</Link>} />
    </header>
  )
}
